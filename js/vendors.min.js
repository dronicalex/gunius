...
function val(inputs) {
	...
	for (let index = 0; index < inputs.length; index++) {
		...
		if (element.classList.contains('_check')) {
		...
		if (element.classList.contains('_phone')) {
...
function vall(mailClass) {
	const EMAIL_REGEXP = /^(([^<>()[\].,;:\s@"]+(\.[^<>()[\].,;:\s@"]+)*)|(".+"))@(([^<>()[\].,;:\s@"]+\.)+[^<>()[\].,;:\s@"]{2,})$/iu;
	const input = mailClass
...
function showErr(inp) {
	inp.parentNode.classList.add('_error')
}
function hidenErr(inp) {
...
// ---------------------------telegramSend---------------------------
// let token = '5949204239:AAFjvIcOgRUqwTMmiK9UunhnqSCgZ6t2fko';
// let chat_id = '-1001729935005';
...
// 			if (val(inputs)) {
// 				let mesage = `<b>${element.dataset.subject}</b> \n`;
// 				for (let index = 0; index < inputs.length; index++) {
// 					const element = inputs[index];
// 					mesage += `<strong>${element.dataset.text}:</strong>   ${element.value}   \n`
// 				}
...
// 				})
// 				// console.log('11')
// 				// document.querySelector('.popup_popup').classList.remove('_active')
// 				// document.querySelector('.popup_thanks').classList.add('_active')
// 			}
// 		}, 200);
...
// async function formSendd(formTwo) {

// 	let formDataOne = new FormData(formTwo)
// 	// formDataOne.append('hr', formTwo.dataset.subject)
// 	let inputs = formTwo.querySelectorAll('input')
// 	if (val(inputs)) {
...
// ---------------------------timer---------------------------
// let endDate = new Date("July 27, 2022 08:00:00").getTime();

// var endDate = new Date(Date.UTC(2023, 1, 8, 18, 0, 0));
...
// 		// document.querySelector("._days").innerHTML = days
// 		document.querySelectorAll("._days").forEach(element => {
// 			element.innerHTML = days
// 		});
// 		console.log('1')
// 		if (hours.toString().length == 1) {
// 			document.querySelectorAll("._hour").forEach(element => {
// 				element.innerHTML = ("0" + hours)
// 			});
...
// 			document.querySelectorAll("._min").forEach(element => {
// 				element.innerHTML = ("0" + mins)
// 			});
// 			// document.querySelector("._min").innerHTML = ("0" + mins)
// 		} else {
// 			document.querySelectorAll("._min").forEach(element => {
// 				element.innerHTML = mins
// 			});
// 			// document.querySelector("._min").innerHTML = mins
// 		}
// 		if (sec.toString().length == 1) {
// 			document.querySelectorAll("._sec").forEach(element => {
// 				element.innerHTML = ("0" + sec)
// 			});
// 			// document.querySelector("._min").innerHTML = ("0" + mins)
// 		} else {
// 			document.querySelectorAll("._sec").forEach(element => {
...
// ---------------------------anim-question---------------------------
let animQuest = document.querySelectorAll('._anim-quest');
for (let index = 0; index < animQuest.length; index++) {
	const element = animQuest[index];

	let quest = element.querySelectorAll('._quest');
	let resp = element.querySelectorAll('._resp');
	...
	for (let index = 0; index < quest.length; index++) {
		...
		// quest[0].classList.add('_active')
		// resp[0].classList.add('_active')
		// if (quest[0].classList.contains('_active')) {
		// 	// itt[index].classList.add('_active')
		// 	resp[0].style.height = `${hh[0]}px`
		// 	setTimeout(() => {
		// 		resp[0].style.height = `auto`
		// 	}, 300);
...
gradientStroke.addColorStop(0, "rgba(92, 66, 245, 1)");
gradientStroke.addColorStop(1, "rgba(0, 150, 255, 1)");
var yourImage = new Image(20, 20);
yourImage.src = '../img/Circle.svg';



let Period = document.querySelector('#Period');
let profit = document.querySelector('#profit');
let Drawdown = document.querySelector('#Drawdown');
...
Drawdown.addEventListener('input', function () {
	calkulator()
})
...
setTimeout(() => {
	...
	for (let index = 0; index < selectOption.length; index++) {
		const element = selectOption[index];
		element.addEventListener('click', function () {
			calkulator()
		})
	}
...
function calkulator() {
	...
	let PeriodsToutes = Period.querySelectorAll('option')
	for (let index = 0; index < PeriodsToutes.length; index++) {



		const element = PeriodsToutes[index];
		console.log(element)
		maxBalanceValue = balanceValue;
		for (let i = 0; i < element.value * 30.5; i++) {
			const el = element[i];
			maxBalanceValue = maxBalanceValue + (maxBalanceValue / 100 * profitValue)
		}

		let m1 = Math.round(parseFloat(maxBalanceValue - balanceValue) * 1000) / 1000
		m1 = m1 - ((maxBalanceValue - balanceValue) / 100 * Drawdown.value)
		m1 = Math.round(parseFloat(m1) * 1000) / 1000;
		console.log(m1)
		if (index <= Period.selectedIndex) {
			profitArray.push(m1)
		} else {
			// profitArray.push(0)
		}
	...
	PeriodSelectedIndex = Period.selectedIndex + 1
	profit.innerHTML = profitArray[Period.selectedIndex + 1]

	console.log(PeriodSelectedIndex)

	for (let index = 0; index < Period.length; index++) {
		const element = PeriodSelectedIndex[index];
		if (index == PeriodSelectedIndex - 1) {
			mass.push(10)
		} else {
			mass.push(0)
		}
	...
	// var segment = chart.getDatasetMeta(0).data[PeriodSelectedIndex];
	// console.log(segment)
	...
	chart.destroy();
	initchart()
	chart.data.datasets[0].data = profitArray
}
		...
		plugins: [{
			afterDatasetUpdate: chart => {
				if (!chart.tooltip.getActiveElements().length) {
					...
					// PeriodSelectedIndex = 
					console.log(PeriodSelectedIndex + 1)
					chart.tooltip.setActiveElements([{
						datasetIndex: 0,
						index: PeriodSelectedIndex
		...
		options: {
			...
			scales: {
				...
				x: {
					...
					// grace: '10%',
					type: 'category',
					labels: ['0', '1 month', '2 month', '3 month', '6 month', '1 year'],
					ticks: {
...
for (let index = 0; index < advantagesItem.length; index++) {
	const element = advantagesItem[index];
	element.style.transitionDelay = `${index / 3}s`
}
advantagesItem = document.querySelectorAll('.conect__item');
for (let index = 0; index < advantagesItem.length; index++) {
	const element = advantagesItem[index];
	element.style.transitionDelay = `${index / 3}s`
}